...
# Dictionary is keyed by date
def prepare_hpi_dict_local(hpi_data):
    dict = {}
    for index, row in hpi_data.iterrows():
        dict[row['Date']] = row['hpi']
    return dict
...
# Dictionary is keyed by date
def prepare_personal_income_dict(unemployment_data):
    dict = {}
    for index, row in unemployment_data.iterrows():
        dict[row['Date']] = row['value']
    #print(dict)
...
# Dictionary is keyed by date
def prepare_rental_vacancy_dict(unemployment_data):
    dict = {}
    for index, row in unemployment_data.iterrows():
        dict[row['Date']] = row['value']
    #print(dict)
...
def predict_top_features(checked_data_sources, SP500_data, interest_data, house_supply_data, lumber_data, hpi_data, unemployment_data, n):
    ...
    for data_source in checked_data_sources:
        ...
        if(data_source == "Unemployment Rate"):
            X['Unemployment'] = get_data_for_specific_dates(dict_unemployment, all_dates)
...
def predict_top_features_local(checked_data_sources, SP500_data, hpi_data, unemployment_data, personal_income_data, rental_vacancy_data, n):
    dict_sp500 = prepare_sp_500_dict(SP500_data)
    dict_personal_income = prepare_personal_income_dict(personal_income_data)
    dict_rental_vacancy = prepare_rental_vacancy_dict(rental_vacancy_data)
    dict_hpi = prepare_hpi_dict_local(hpi_data)
    dict_unemployment = prepare_unemployment_dict(unemployment_data)
    ## Assuming that the hpi data is always present
    ## TODO(chirag): Think of what needs to be done where hpi data is not present
    all_dates = dict_hpi.keys()
    ...
    for data_source in checked_data_sources:
        if(data_source == "S&P 500"):
            X['SP500'] = get_data_for_specific_dates(dict_sp500, all_dates)
        if(data_source == "Personal Income"):
            X['Personal Income'] = get_data_for_specific_dates(dict_personal_income, all_dates)
        if(data_source == "Rental Vacancy"):
            X['Rental Vacancy'] = get_data_for_specific_dates(dict_rental_vacancy, all_dates)
        if(data_source == "Unemployment Rate"):
            X['Unemployment'] = get_data_for_specific_dates(dict_unemployment, all_dates)
...
def predict_future_values_local(checked_data_sources, SP500_data, interest_data, house_supply_data, lumber_data, hpi_data, unemployment_data):
    dict_sp500 = prepare_sp_500_dict(SP500_data)
    dict_interest = prepare_interest_dict(interest_data)
    dict_house_supply = prepare_house_supply_dict(house_supply_data)
    dict_lumber = prepare_lumber_dict(lumber_data)
